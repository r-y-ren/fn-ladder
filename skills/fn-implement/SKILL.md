...
2. **轻量对账**（新会话/压缩/中途接手必做）：跑两条全局命令——grep 桩标记、全量测试——与 functions.md 函数大表对照；结果只在对话里报告，不落盘、不加文档节。**对不上先修文档再干活**，防止上个会话的谎报传染。
...
每批验收点达成 → 汇报核验输出 → **停，显式提示三项：继续下一批 B<N>（说"继续"即可）/ 修订本批产物 / 裁决 blocked 项**；用户未显式选择前不推进。批间是天然会话边界：可 `/clear` 或压缩后新开会话，靠轻量对账无缝续批。
